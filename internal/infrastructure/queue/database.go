...
func NewDatabaseWorker(db *gorm.DB, cfg config.QueueConfig, registry *queue.HandlerRegistry) *DatabaseWorker {
	...
	for {
		...
		if err != nil || job == nil {
			if err != nil && !errors.Is(err, context.Canceled) {
				log.Printf("Database queue reservation failed: %v", err)
			}
...
func (w *DatabaseWorker) process(workerCtx context.Context, job *databaseJob) {
	...
	if !ok {
		if err := w.fail(job, fmt.Errorf("no handler registered for %q", job.Type)); err != nil {
			log.Printf("Database queue failure transition failed for job %s: %v", job.ID, err)
		}
		return
	}
	...
	if err != nil {
		if updateErr := w.fail(job, err); updateErr != nil {
			log.Printf("Database queue failure transition failed for job %s: %v", job.ID, updateErr)
		}
		return
	}
	if err := w.complete(job); err != nil {
		log.Printf("Database queue completion failed for job %s: %v", job.ID, err)
	}
}
...
func (w *DatabaseWorker) heartbeat(ctx context.Context, job *databaseJob, done <-chan struct{}) {
	...
	for {
		...
		case <-done:
			return
		case <-ticker.C:
			w.db.WithContext(ctx).Model(&databaseJob{}).
				Where("id = ? AND status = ? AND lease_token = ?", job.ID, jobStatusActive, *job.LeaseToken).
				Update("leased_until", time.Now().Add(w.reservation))
		}
	}
}
